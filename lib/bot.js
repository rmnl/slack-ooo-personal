...
class Bot {
  ...
  handleMessage (message) {
    ...
    const displayName = (user && user.profile && user.profile.display_name) ? `@${user.profile.display_name}` : false;
    ...
    if (userName === `@${this.bot.identity.name}`) {
      ...
      logger.info('You are mentioning yourself!');
    } else if (user.is_bot && !this.config.app.respond.bot) {
    // if (user.is_bot && !this.config.app.respond.bot) {
      ...
      if ((message.event === 'direct_message') && this.config.app.respond.dm) {
        if ((this.responded.get(message.channel) || 0) < Date.now()) {
          ...
          response = {
            text: this.config.app.message.replace('${user}', displayName || userName)
            // attachments: [
            //   {
            //     color: 'BEDEAD',
        ...
        if ((this.responded.get(message.channel) || 0) < Date.now()) {
          response = {
            text: this.config.app.channel_message.replace('${user}', displayName || userName),
            attachments: [
              {
                color: 'BEDEAD',
                fallback: "",
                text: "",
                footer: `
                  Please ignore this message if you were already aware. This message will only reappear if
                  I get mentioned again after ${this.config.app.reminder} minutes from this moment.
                `
              }
            ]
          };
